...
func init() {
	// Switch to root to use test data
	os.Chdir("../../")
}
...
func TestRun(t *testing.T) {
	testCases := []struct {
			...
			expected: "testdata/dir.log\ntestdata/dir2/script.sh\n"},
		{
			...
			expected: "testdata/dir.log\n"},
		{
			name: "FilterExtensionsSizeMatch", root: "testdata",
...
func TestRunDelExtension(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			var (
				buffer   bytes.Buffer
				logBufer bytes.Buffer
			)

			tc.cfg.wLog = &logBufer
			...
			lines := bytes.Split(logBufer.Bytes(), []byte("\n"))
